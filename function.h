...
void display_Menu(){
         ...
         do{
        printf("--------------Welcome to Customer Management System---------------\n\n");
        printf("\t1. Add a new customer\n");
        printf("\t2. Display information of all customers\n");
        printf("\t3. Update the information of a customer by id\n");
        printf("\t4. Delete a customer by id\n");
        printf("\t5. Display information of a specific customer by id\n");
        printf("\t6. Display all blacklist customer\n");
        printf("\t7. Exit\n");
        printf("-------------------------------------------------------------------\n\n");
...
void option(int choice){
    switch(choice){
        ...
        case 6: printf("You chose 6\n");
                readFile();
                printf("-------------Blacklist customer---------------\n");
                displayAllblacklist();
                reset();
                    break;
        case 7: printf("You chose 7\n");
                readFile();
                writeFile();
                exit(0);
                break;
...
void displayAllcustomer() {
    //    print information
    int i;
    printf("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n",ID,Name,Item_name,Quantity,Price,Discount,recieve_date,order_date,Delivery_fee,address,Phone_number,Delivery_man,Deliveryman_phone_number);
    for (i=0; i<last_index; i++){
        printf("%d\t%s\t%s\t\t%d\t\t%d\t\t%d\t\t%s\t%s\t%d\t\t%s\t%s\t%s\t\t%s\n",c[i].id,c[i].name,c[i].item_name,c[i].quality,c[i].price,c[i].discount,c[i].recieved_date,c[i].ordered_date,c[i].delivery_fee,c[i].address,c[i].phone_number,c[i].deliveryman_name,c[i].deliveryman_phone_number);
    }
}
...
displayAllblacklist(){
    ...
    int j=0,blacklist=0;
    fscanf(file, "%s %s %s %s %s %s %s %s %s %s %s %s %s",ID,Name,Item_name,Quantity,Price,Discount,order_date,recieve_date,Delivery_fee,address,Phone_number,Delivery_man,Deliveryman_phone_number);
    while(fscanf(file,"%s %s %s %s %s %s %s %s %s %s %s %s %s",str_id,bc[j].name,bc[j].item_name,str_quan,str_price,str_discount,bc[j].ordered_date,bc[j].recieved_date,str_delFee,bc[j].address,bc[j].phone_number,bc[j].deliveryman_name,bc[j].deliveryman_phone_number) != EOF) {
        bc[j].id = atoi(str_id);
        bc[j].quality = atoi(str_quan);
        bc[j].price = atoi(str_price);
        bc[j].discount = atoi(str_discount);
        bc[j].delivery_fee = atoi(str_delFee);
        j++;
        blacklist = j;
    }
    int i;
    for (i=0; i<last_index; i++){
        printf("%d\t%s\t%s\t\t%d\t\t%d\t\t%d\t\t%s\t%s\t%d\t\t%s\t%s\t%s\t\t%s\n",bc[i].id,bc[i].name,bc[i].item_name,bc[i].quality,bc[i].price,bc[i].discount,bc[i].ordered_date,bc[i].recieved_date,bc[i].delivery_fee,bc[i].address,bc[i].phone_number,bc[i].deliveryman_name,bc[i].deliveryman_phone_number);
    }
    fclose(file);
}
